...
function( n_x, Version, loc_x, Data_Geostat, FieldConfig, ObsConfig, ObsModel, Aniso, NN, X_a_xjt, X_b_xjt ){
  ...
  if(Version%in%c("growth_v1a","growth_v1b","growth_v1c","growth_v2a","growth_v2a_fix","growth_v2a_bridge","growth_v2b","growth_v2c","growth_v2d","growth_v2e")){
    # Data
    if( Version %in% c("growth_v1a") ) TmbData = list("n_i"=nrow(Data_Geostat), "n_s"=spde$n.spde, "n_j"=n_x, , "n_t"=diff(range(unique(Data_Geostat[,'Year'])))+1, "Aniso"=Aniso, "ObsModel"=ObsModel, "w_i"=Data_Geostat[,'WEIGHT_KG'], "l_i"=Data_Geostat[,'LENGTH_CM'], "J_i"=NN$nn.idx[,1]-1, "T_i"=Data_Geostat[,'Year']-min(Data_Geostat[,'Year']), "n_tri"=nrow(mesh$graph$tv), "Tri_Area"=Tri_Area, "E0"=E0, "E1"=E1, "E2"=E2, "TV"=TV-1, "G0_inv"=as(diag(1/diag(spde$param.inla$M0)),"dgTMatrix"), "G0"=spde$param.inla$M0, "G1"=spde$param.inla$M1, "G2"=spde$param.inla$M2)
    if( Version %in% c("growth_v1b") ) TmbData = list("n_i"=nrow(Data_Geostat), "n_s"=spde$n.spde, "n_j"=n_x, "n_t"=diff(range(unique(Data_Geostat[,'Year'])))+1, "Aniso"=Aniso, "ObsModel"=ObsModel, "w_i"=Data_Geostat[,'WEIGHT_KG'], "l_i"=Data_Geostat[,'LENGTH_CM'], "J_i"=NN$nn.idx[,1]-1, "T_i"=Data_Geostat[,'Year']-min(Data_Geostat[,'Year']), "S_i"=sapply(Data_Geostat[,'Sex'],FUN=switch,0,1,0.5), "n_tri"=nrow(mesh$graph$tv), "Tri_Area"=Tri_Area, "E0"=E0, "E1"=E1, "E2"=E2, "TV"=TV-1, "G0_inv"=as(diag(1/diag(spde$param.inla$M0)),"dgTMatrix"), "G0"=spde$param.inla$M0, "G1"=spde$param.inla$M1, "G2"=spde$param.inla$M2)
    if( Version %in% c("growth_v1c","growth_v2a") ) TmbData = list("n_i"=nrow(Data_Geostat), "n_s"=spde$n.spde, "n_x"=n_x, "n_t"=diff(range(unique(Data_Geostat[,'Year'])))+1, "n_j"=dim(X_a_xjt)[2], "Aniso"=Aniso, "ObsModel"=ObsModel, "w_i"=Data_Geostat[,'WEIGHT_KG'], "l_i"=Data_Geostat[,'LENGTH_CM'], "X_i"=NN$nn.idx[,1]-1, "T_i"=Data_Geostat[,'Year']-min(Data_Geostat[,'Year']), "S_i"=sapply(Data_Geostat[,'Sex'],FUN=switch,0,1,0.5), "X_xjt"=X_a_xjt, "n_tri"=nrow(mesh$graph$tv), "Tri_Area"=Tri_Area, "E0"=E0, "E1"=E1, "E2"=E2, "TV"=TV-1, "G0_inv"=as(diag(1/diag(spde$param.inla$M0)),"dgTMatrix"), "G0"=spde$param.inla$M0, "G1"=spde$param.inla$M1, "G2"=spde$param.inla$M2)
    if( Version %in% c("growth_v2a_fix") ) TmbData = list("n_i"=nrow(Data_Geostat), "n_s"=spde$n.spde, "n_x"=n_x, "n_t"=diff(range(unique(Data_Geostat[,'Year'])))+1, "n_j"=dim(X_a_xjt)[2], "Aniso"=Aniso, "ObsModel"=ObsModel, "FieldConfig"=FieldConfig, "w_i"=Data_Geostat[,'WEIGHT_KG'], "l_i"=Data_Geostat[,'LENGTH_CM'], "X_i"=NN$nn.idx[,1]-1, "T_i"=Data_Geostat[,'Year']-min(Data_Geostat[,'Year']), "S_i"=sapply(Data_Geostat[,'Sex'],FUN=switch,0,1,0.5), "X_xjt"=X_a_xjt, "n_tri"=nrow(mesh$graph$tv), "Tri_Area"=Tri_Area, "E0"=E0, "E1"=E1, "E2"=E2, "TV"=TV-1, "G0_inv"=as(diag(1/diag(spde$param.inla$M0)),"dgTMatrix"), "G0"=spde$param.inla$M0, "G1"=spde$param.inla$M1, "G2"=spde$param.inla$M2)
    if( Version %in% c("growth_v2a_bridge") ) TmbData = list("n_i"=nrow(Data_Geostat), "n_s"=spde$n.spde, "n_x"=n_x, "n_t"=diff(range(unique(Data_Geostat[,'Year'])))+1, "n_j"=dim(X_a_xjt)[2], "Aniso"=Aniso, "ObsModel"=ObsModel, "FieldConfig"=FieldConfig, "w_i"=Data_Geostat[,'WEIGHT_KG'], "l_i"=Data_Geostat[,'LENGTH_CM'], "X_i"=NN$nn.idx[,1]-1, "T_i"=Data_Geostat[,'Year']-min(Data_Geostat[,'Year']), "S_i"=sapply(Data_Geostat[,'Sex'],FUN=switch,0,1,0.5), "X_xjt"=X_a_xjt, "n_tri"=nrow(mesh$graph$tv), "Tri_Area"=Tri_Area, "E0"=E0, "E1"=E1, "E2"=E2, "TV"=TV-1, "G0_inv"=as(diag(1/diag(spde$param.inla$M0)),"dgTMatrix"), "G0"=spde$param.inla$M0, "G1"=spde$param.inla$M1, "G2"=spde$param.inla$M2)
    if( Version %in% c("growth_v2b") ) TmbData = list("n_i"=nrow(Data_Geostat), "n_s"=spde$n.spde, "n_x"=n_x, "n_t"=diff(range(unique(Data_Geostat[,'Year'])))+1, "n_j_a"=dim(X_a_xjt)[2], "n_j_b"=dim(X_b_xjt)[2], "Aniso"=Aniso, "ObsModel"=ObsModel, "FieldConfig"=FieldConfig, "w_i"=Data_Geostat[,'WEIGHT_KG'], "l_i"=Data_Geostat[,'LENGTH_CM'], "X_i"=NN$nn.idx[,1]-1, "T_i"=Data_Geostat[,'Year']-min(Data_Geostat[,'Year']), "S_i"=sapply(Data_Geostat[,'Sex'],FUN=switch,0,1,0.5), "X_a_xjt"=X_a_xjt, "X_b_xjt"=X_b_xjt, "n_tri"=nrow(mesh$graph$tv), "Tri_Area"=Tri_Area, "E0"=E0, "E1"=E1, "E2"=E2, "TV"=TV-1, "G0_inv"=as(diag(1/diag(spde$param.inla$M0)),"dgTMatrix"), "G0"=spde$param.inla$M0, "G1"=spde$param.inla$M1, "G2"=spde$param.inla$M2)
